...
| CWE 编号 | 描述 |
|---------|------|
| CWE-362 | 并发执行共享资源的同步问题（Race Condition） |
| CWE-367 | 时间窗口检查（TOCTOU） |
| CWE-820 | 缺少同步 |
| CWE-364 | 信号处理程序中的竞态条件 |
| CWE-366 | 通道内的竞态条件 |
| CWE-412 | 未预期的信号处理器通道 |
| CWE-609 | 双重检查锁定 |
| CWE-821 | 正确的同步更新缺失 |
...
```bash
# 场景：程序检查文件后打开
# 1. 程序检查 /tmp/safe_file 是否存在
# 2. 攻击者快速替换为指向 /etc/passwd 的符号链接
# 3. 程序打开符号链接，实际读取 /etc/passwd

# CVE 案例：
# - CVE-2008-2958: TOCTOU 绕过符号链接保护
# - CVE-2008-1570: 同上
```
